...
'
'           The licence, help and about forms are RC6 form windows without a titlebar nor controls, displaying a fullsize image.
'           Standard VB6 forms are used for the preference form.
...
'           resized dynamically when the form is dragged.
'
...
Public Sub mainRoutine(ByVal restart As Boolean)
    ...
    gblStartupFlg = True
    gblWidgetName = "Sun Widget"
    thisPSDFullPath = App.Path & "\Res\Sun Widget VB6.psd"
    ...
    menuForm.mnuAbout.Caption = "About Sun " & gblRichClientEnvironment & " Cairo " & gblCodingEnvironment & " widget"
    ...
    Call readSettingsFile("Software\Sun", gblSettingsFile)
...
'    ' Set the opacity of the widget, passing just this one global variable to a public property within the class
    SunWidget.opacity = Val(gblOpacity) / 100
    ...
    Call SubclassForm(fMain.SunForm.hWnd, ObjPtr(fMain.SunForm))
...
Private Sub checkFirstTime()
   ...
   On Error GoTo checkFirstTime_Error
    ...
    If gblFirstTimeRun = "true" Then
        ...
        sPutINISetting "Software\Sun", "firstTimeRun", gblFirstTimeRun, gblSettingsFile
    End If
...
'    add Resources to the global ImageList that are not being pulled from the PSD directly
    ...
    Cairo.ImageList.AddImage "help", App.Path & "\Resources\images\Help.png"
...
Public Sub adjustMainControls(Optional ByVal licenceState As Integer)
   ...
   On Error GoTo adjustMainControls_Error
    ...
    SunWidget.SkewDegrees = CDbl(gblSkewDegrees)
    ...
    If gblMonitorCount > 1 And (LTrim$(gblMultiMonitorResize) = "1" Or LTrim$(gblMultiMonitorResize) = "2") Then
        If widgetMonitorStruct.IsPrimary = True Then
            SunWidget.Zoom = (Val(gblWidgetPrimaryHeightRatio))
        Else
            SunWidget.Zoom = (Val(gblWidgetSecondaryHeightRatio))
        End If
    Else
        SunWidget.Zoom = Val(gblWidgetSize) / 100
    End If
    ...
    If gblShowTaskbar = "0" Then
        fMain.SunForm.ShowInTaskbar = False
    Else
        fMain.SunForm.ShowInTaskbar = True
    End If
   ...
   If gblPreventDragging = "0" Then
    ...
    Else
        ...
        SunWidget.Locked = True ' this is just here for continuity's sake, it is also set at the time the control is selected
    End If
...
Public Sub setAlphaFormZordering()
   ...
   On Error GoTo setAlphaFormZordering_Error
    ...
    If Val(gblWindowLevel) = 0 Then
        Call SetWindowPos(fMain.SunForm.hWnd, HWND_BOTTOM, 0&, 0&, 0&, 0&, OnTopFlags)
    ElseIf Val(gblWindowLevel) = 1 Then
        Call SetWindowPos(fMain.SunForm.hWnd, HWND_TOP, 0&, 0&, 0&, 0&, OnTopFlags)
    ElseIf Val(gblWindowLevel) = 2 Then
        Call SetWindowPos(fMain.SunForm.hWnd, HWND_TOPMOST, 0&, 0&, 0&, 0&, OnTopFlags)
    End If
...
'        gblEnableSounds = fGetINISetting(Location, "enableSounds", gblSettingsFile)
        ...
        ' other
        gblWidgetHighDpiXPos = fGetINISetting("Software\Sun", "widgetHighDpiXPos", gblSettingsFile)
        gblWidgetHighDpiYPos = fGetINISetting("Software\Sun", "widgetHighDpiYPos", gblSettingsFile)
        gblWidgetLowDpiXPos = fGetINISetting("Software\Sun", "widgetLowDpiXPos", gblSettingsFile)
        gblWidgetLowDpiYPos = fGetINISetting("Software\Sun", "widgetLowDpiYPos", gblSettingsFile)
...
Private Sub getToolSettingsFile()
    ...
    gblSettingsDir = fSpecialFolder(feUserAppData) & "\Sun-" & gblRichClientEnvironment & "-Widget-" & gblCodingEnvironment & "" ' just for this user alone
...
Private Sub createRCFormsOnCurrentDisplay()
    ...
    With New_c.Displays(1) 'get the current Display
      Call fMain.initAndCreateSunForm(.WorkLeft, .WorkTop, imageWidth, imageHeight, gblWidgetName)
    End With
...
Private Sub handleUnhideMode(ByVal thisUnhideMode As String)
    ...
    If thisUnhideMode = "unhide" Then     'parse the command line
        gblUnhide = "true"
        sPutINISetting "Software\Sun", "unhide", gblUnhide, gblSettingsFile
